...
def index():
    ...
    if (request.method =='POST'):
        ...
        if submit_type == "textinput" and text and text.strip(): # avoids crash on None
            msg = ("You", text)
            Messages.append(msg)
            if len(Messages) == 1 :
            ...
            if "open" in text.lower().strip():       
                if any(command in text.lower() for command in command_dict.keys()):
                    ...
                    FinalOutput = f"Sorry , Couldn't find Redirected to Google Search for {name}..."
        ...
        if submit_type == "voiceinput": 
            recognizer = sr.Recognizer()
            engine = pyttsx3.init()
            voiceloop=True
            engine.stop()
            engine.say("Voice input activated")
            engine.runAndWait()
            while(True):
                with sr.Microphone() as source:
                    audio = recognizer.listen(source)
                try:
                    voicetext = recognizer.recognize_google(audio)
                    if "open" in voicetext.lower().strip():       
                        if any(command in voicetext.lower() for command in command_dict.keys()):
                            for command in command_dict:
                                if command in voicetext.lower():
                                    matched_command = command
                                    link= command_dict[matched_command]    
                            webbrowser.open_new(link)
                            engine.stop()
                            engine.say(f"Successfully Opened {matched_command.replace('open', '')}")
                            engine.runAndWait()
                            
                        else:
                            webname=voicetext.lower().replace("open","").strip()
                            webbrowser.open_new(f"https://www.google.com/search?q={webname}")
                            engine.stop()
                            engine.say(f"Sorry , Couldn't find, Redirected to Google Search for {webname}...")
                            engine.runAndWait()
                                    
                    else:
                            
                        voiceresponse = co.generate(
                            prompt=voicetext,
                            max_tokens=150
                        )
                        engine.stop()
                        engine.say(voiceresponse.generations[0].text)
                        engine.runAndWait()
            ...
            engine.stop()
            engine.say("Voice input deactivated")
            engine.runAndWait()
            voiceloop=False
    ...
    return render_template('index.html',Messages=Messages, File_List=File_List, voiceloop=voiceloop)
...
if __name__=="__main__":
    ...
    app.run(debug=False, use_reloader=False, host=os.getenv("HOST"), port=5000)
